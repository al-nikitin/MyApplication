...
public class MainActivity extends AppCompatActivity {
    ...
    String input = "ioe dec 890 if qk > -10\n" +
            "gif inc -533 if qt <= 7\n" +
            "itw dec 894 if t != 0\n" +
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        while (input.contains("\n")) {
            ...
            instructions.add(new Instruction(input.substring(0, index + 1)));
            if (index == input.length() - 1) {
                input = "";
            } else input = input.substring(index + 1);
        }

        for (Instruction instruction : instructions) {
            if (checkCondition(instruction)) doOperation(instruction);
        }

        for (String register : registers.keySet()) {
            if (registers.get(register) > max) max = registers.get(register);
        }
        System.out.println(registers);
        tv.setText(max+"");
    }

    boolean checkCondition(Instruction instruction) {
        boolean conditionIsTrue = false;
        int valueToCheck = registers.get(instruction.registerToCheck);
        int valueToCheckAgainst = instruction.valueCheck;
        switch (instruction.condition) {
            case ">":
                if (valueToCheck > valueToCheckAgainst) conditionIsTrue = true;
                break;
            case ">=":
                if (valueToCheck >= valueToCheckAgainst) conditionIsTrue = true;
                break;
    ...
    void doOperation(Instruction instruction) {
        int initial = registers.get(instruction.registerToModify);
        int value = instruction.valueModify;
        if (instruction.operation.equals("dec")) value = -value;
        int newValue = initial + value;
        registers.put(instruction.registerToModify, newValue);
        if (newValue > max) max = newValue;
    }

    class Instruction {
        String registerToModify, operation, registerToCheck, condition;
        int valueModify, valueCheck;
        public Instruction(String instruction) {
            this.registerToModify = instruction.substring(0, instruction.indexOf(" "));
            if (!(registers.containsKey(this.registerToModify))) registers.put(this.registerToModify, 0);
            instruction = instruction.substring(instruction.indexOf(" ") + 1);
            this.operation = instruction.substring(0, instruction.indexOf(" "));
            instruction = instruction.substring(instruction.indexOf(" ") + 1);
            this.valueModify = Integer.parseInt(instruction.substring(0, instruction.indexOf(" ")));
            instruction = instruction.substring(instruction.indexOf(" ") + 4);
            this.registerToCheck = instruction.substring(0, instruction.indexOf(" "));
            instruction = instruction.substring(instruction.indexOf(" ") + 1);
            this.condition = instruction.substring(0, instruction.indexOf(" "));
            instruction = instruction.substring(instruction.indexOf(" ") + 1);
            this.valueCheck = Integer.parseInt(instruction.substring(0, instruction.indexOf("\n")));
        }
    }
}
